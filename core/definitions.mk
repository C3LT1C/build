...
KERNEL_MODULES_OUT := $(OUT_DIR)/$(CELTIC_PRODUCT)/system/lib/modules

MKBOOTIMG_ARGS := --kernel $(OUT_DIR)/$(CELTIC_PRODUCT)/$(TARGET_ZIMAGE) --ramdisk $(OUT_DIR)/$(CELTIC_PRODUCT)/boot.img-ramdisk.gz
...
$(MKBOOTIMG_ARGS) += $(OUT_DIR)/$(CELTIC_PRODUCT)/dtb
endif
...
define cp-zimage
echo "$(target_arch)";\
mkdir -p $(OUT_DIR)/$(CELTIC_PRODUCT);\
cp $(PRODUCT_KERNEL_SOURCE)/arch/$(ARCH)/boot/$(TARGET_ZIMAGE) $(OUT_DIR)/$(CELTIC_PRODUCT)/$(TARGET_ZIMAGE)
endef

define cp-zip-files
cp -r $(ZIP_FILES_DIR)/* $(OUT_DIR)/$(CELTIC_PRODUCT);\
echo "\033[32m Zipping the following files:\033[0m";\
ziprepacks=`find $(OUT_DIR)/$(CELTIC_PRODUCT) -type f`;\
for i in $$ziprepacks; do echo "\033[32m $$i \033[0m"; done;\
fi
endef

define build-zip
mkdir -p $(ANDROID_BUILD_TOP)/Zip-Files/$(CELTIC_PRODUCT);\
cd $(OUT_DIR)/$(CELTIC_PRODUCT);\
zip -qr9 $(ANDROID_BUILD_TOP)/Zip-Files/$(CELTIC_PRODUCT)/$(PACKAGE_TARGET_NAME) ./*;\
cd $(ANDROID_BUILD_TOP)
endef

define make_dtb
build/tools/dtbToolCM -2 -o $(OUT_DIR)/$(CELTIC_PRODUCT)/dtb -s 2048 -p $(PRODUCT_KERNEL_SOURCE)/scripts/dtc/ $(PRODUCT_KERNEL_SOURCE)/$(DTB_DIR)/
endef

define make_ramdisk
build/tools/mkbootfs $(BOARD_RAMDISK_DIR) | gzip > $(OUT_DIR)/$(CELTIC_PRODUCT)/boot.img-ramdisk.gz
endef

define make_boot
build/tools/mkbootimg $(MKBOOTIMG_ARGS) --output $(OUT_DIR)/$(CELTIC_PRODUCT)/boot.img
endef

define clear_boot-ramdisk
rm $(OUT_DIR)/$(CELTIC_PRODUCT)/boot.img-ramdisk.gz;\
rm $(OUT_DIR)/$(CELTIC_PRODUCT)/$(TARGET_ZIMAGE)
endef
