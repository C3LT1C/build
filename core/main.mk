...
kernel:
	...
	$(cp-zimage)
	@echo "\033[32m Copied $(PRODUCT_KERNEL_SOURCE)/arch/$(ARCH)/boot/$(TARGET_ZIMAGE) to $(OUT_DIR)/$(CELTIC_PRODUCT) \033[0m"

.PHONY: kernelclean
kernelclean:
	@echo "\033[32m Cleaning source \033[0m"
	make -C $(PRODUCT_KERNEL_SOURCE) mrproper > /dev/null
	@echo "Clearing device repacking dir: $(OUT_DIR)/$(CELTIC_PRODUCT)"
	$(shell rm -rf $(OUT_DIR)/$(CELTIC_PRODUCT)/*)
...
ifeq ($(build_type),kernel)
	@echo
	@echo "\033[32m $(OUT_DIR)/$(CELTIC_PRODUCT)/$(TARGET_ZIMAGE) built successful\033[0m"
else
	$(hide) md5sum ./Zip-Files/$(CELTIC_PRODUCT)/$(PACKAGE_TARGET_NAME) | sed 's\./Zip-Files/$(CELTIC_PRODUCT)/\\' > ./Zip-Files/$(CELTIC_PRODUCT)/$(PACKAGE_TARGET_NAME).md5
	@echo
	@echo "\033[32m ./Zip-Files/$(CELTIC_PRODUCT)/$(PACKAGE_TARGET_NAME) built successful\033[0m"
	@echo "md5: `cat ./Zip-Files/$(CELTIC_PRODUCT)/$(PACKAGE_TARGET_NAME).md5 | cut -d ' ' -f 1`"
endif
